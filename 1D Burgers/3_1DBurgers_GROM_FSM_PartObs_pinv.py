# -*- coding: utf-8 -*-
"""
FSM estimation of global eddy viscosity parameter from sparse measurements with pseudo-inverse approach 
...
seed(2) #you might change the seed
...
z = np.linalg.pinv(Phi_s) @ (uObs- um[loc].reshape(-1,1)) #using pseudo-inverse
...
plt.plot(x[loc],uObs[:,0], 'o', label=r'$t=0.25$',linewidth=3)
plt.plot(x[loc],uObs[:,1], 'o', label=r'$t=0.50$',linewidth=3)
...
plt.ylim([-0.5,1.5])
plt.savefig('./Plots/fig5.png', dpi = 500, bbox_inches = 'tight')
plt.show()
...
plt.savefig('./Plots/fig6.png', dpi = 500, bbox_inches = 'tight')
fig.show()
...
plt.savefig('./Plots/fig7.png', dpi = 500, bbox_inches = 'tight')
